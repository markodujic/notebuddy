...
## Expo/Metro-Server niemals selbst starten

Der Agent darf **niemals** `npx expo start` oder vergleichbare Metro-Bundler-Kommandos selbst ausführen. Der User startet den Metro-Server manuell, wenn nötig. Der Agent beschränkt sich auf Code-Änderungen, TypeScript-Checks (`npx tsc --noEmit`) und Build-Verify (`npm run verify:reanimated`).
